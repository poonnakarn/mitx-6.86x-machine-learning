...
def compute_probabilities(X, theta, temp_parameter):
    ...
    """
    #YOUR CODE HERE
    d = theta @ np.transpose(X) / temp_parameter
    C = np.amax(d, axis=0)
    exp_X = np.exp(d-C)
    s = np.sum(exp_X, axis=0)
    H = exp_X/s
    return H
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    cost = (-1/n) * cost
    reg_term = lambda_factor/2 * np.sum(np.power(theta,2))
    cost = cost + reg_term
    return cost
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    M = np.zeros([k,n])
    M[Y, np.arange(n)] = 1
    M = M - compute_probabilities(X, theta, temp_parameter) #dim k x n
    d_theta = (-1/(temp_parameter*n))*(M@X) + lambda_factor*theta #dim k x d + k x d
