...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if r <= epsilon:
        ...
        #choose Qmax
        idx = np.unravel_index(q_func[state_1, state_2, :, :].argmax(), q_func[state_1, state_2].shape)
        action_index = idx[0]
        object_index = idx[1]
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    Returns:
        None
    """
    idx = np.unravel_index(q_func[next_state_1, next_state_2, :, :].argmax(), q_func[next_state_1, next_state_2].shape)
    
    if terminal is True:
        f_maxq = 0
    else:
        f_maxq = q_func[next_state_1, next_state_2, idx[0], idx[1]]
        
    # TODO Your code here
    q_func[current_state_1, current_state_2, action_index,
           object_index] = (1-ALPHA)*q_func[current_state_1, current_state_2, action_index, object_index] + ALPHA*(reward + GAMMA*f_maxq)
